...
export default function ProductCard({ product, currentVariant, onVariantChange }) {
    ...
    return (
        <div
            ...
            className="group h-full flex flex-col bg-white border border-gray-100/60 rounded-2xl overflow-hidden hover:border-[#0072bc]/30 hover:shadow-[0_20px_40px_-15px_rgba(0,114,188,0.1)] transition-all duration-300 cursor-pointer"
        >
                ...
                <div className="mt-auto mb-5">
                    ...
                    <div className="flex flex-wrap gap-0.5">
                        {product.variants.map((variant) => {
                            const isSelected = currentVariant.type === variant.type;
                            return (
                                <button
                                    key={variant.type}
                                    onClick={(e) => {
                                    ...
                                    className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all border ${isSelected
                                        ? 'bg-[#0072bc] border-[#0072bc] text-white shadow-md shadow-blue-500/20'
